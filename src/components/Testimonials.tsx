...
export function Testimonials() {
  return (
    <section id="testimonials" className="relative overflow-hidden bg-gradient-to-b from-sand/40 to-cream py-28">
      <div className="mx-auto max-w-7xl px-6">
        <div className="mb-14 max-w-2xl">
          <p className="text-xs uppercase tracking-[0.3em] text-gold">— Loved by thousands</p>
          <h2 className="mt-3 font-display text-5xl text-primary">Whispers of <span className="italic text-gradient-gold">wellness</span>.</h2>
        </div>
        <div className="grid gap-6 md:grid-cols-3">
          {T.map((t, i) => (
            <motion.figure
              ...
              whileHover={{ y: -8 }}
              className="rounded-3xl border border-gold/20 bg-cream p-8 shadow-soft"
            >
              <div className="flex gap-1 text-gold">
                {Array.from({length:5}).map((_,k)=> <Star key={k} className="h-4 w-4 fill-current" />)}
              </div>
              <blockquote className="mt-4 font-display text-2xl leading-snug text-primary">"{t.text}"</blockquote>
